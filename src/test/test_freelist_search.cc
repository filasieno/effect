...
static Void ResetBitField(__m256i* bitField) {
    ...
    priv::set_alloc_freelist_mask(bitField, 255);
}
...
CThread co_main() noexcept {
    ...
    ResetBitField(bitField);
    set_alloc_freelist_mask(bitField, 0);
    set_alloc_freelist_mask(bitField, 1);
    clear_alloc_freelist_mask(bitField, 0);
    assert(!get_alloc_freelist_mask(bitField, 0));
    assert(get_alloc_freelist_mask(bitField, 1));
    assert(find_alloc_freelist_index(bitField, 1) == 1);
    ...
    ResetBitField(bitField);
    for (int i = 0; i <= 10; ++i) set_alloc_freelist_mask(bitField, i);
    clear_alloc_freelist_mask(bitField, 2);
    clear_alloc_freelist_mask(bitField, 4);
    clear_alloc_freelist_mask(bitField, 6);
    clear_alloc_freelist_mask(bitField, 8);
    unsigned req1 = 1;  // requiredBin=0
    assert(find_alloc_freelist_index(bitField, req1) == 0);  // 0 is set
    unsigned req3 = 33;  // requiredBin=1
    assert(find_alloc_freelist_index(bitField, req3) == 1);  // 1 set, 2 cleared

    // Test 21: requiredBin=127, bit127 set (end of second word)
    std::print("Test 21: requiredBin=127, bit127 set (end of second word)\n");
    ResetBitField(bitField);
    set_alloc_freelist_mask(bitField, 127);
    unsigned req127 = 32 * 127 + 1;
    assert(find_alloc_freelist_index(bitField, req127) == 127);

    // Test 22: requiredBin=128, bit128 set (start of third word)
    std::print("Test 22: requiredBin=128, bit128 set (start of third word)\n");
    ResetBitField(bitField);
    set_alloc_freelist_mask(bitField, 128);
    unsigned req128 = 32 * 128 + 1;
    assert(find_alloc_freelist_index(bitField, req128) == 128);

    // Test 23: no bits set from 200 to 254, falls to 255
    std::print("Test 23: no bits set from 200 to 254, falls to 255\n");
    ResetBitField(bitField);
    for (int i = 0; i < 200; ++i) set_alloc_freelist_mask(bitField, i);
    unsigned req200 = 32 * 200 + 1;
    assert(find_alloc_freelist_index(bitField, req200) == 255);  // since 200-254 not set

    // Test 24: verify Get after Set and Clear across bytes
    std::print("Test 24: verify Get after Set and Clear across bytes\n");
    ResetBitField(bitField);
    set_alloc_freelist_mask(bitField, 7);   // last bit in byte 0
    set_alloc_freelist_mask(bitField, 8);   // first in byte 1
    assert(get_alloc_freelist_mask(bitField, 7));
    assert(get_alloc_freelist_mask(bitField, 8));
    clear_alloc_freelist_mask(bitField, 7);
    assert(!get_alloc_freelist_mask(bitField, 7));
    assert(get_alloc_freelist_mask(bitField, 8));
    ...
    ResetBitField(bitField);
    clear_alloc_freelist_mask(bitField, 255);  // Temporarily clear wild for test (though design says always set)
    assert(find_alloc_freelist_index(bitField, 0) == 255);  // Should return 255 even if cleared, per code
    set_alloc_freelist_mask(bitField, 255);  // Restore
    ...
    ResetBitField(bitField);
    clear_alloc_freelist_mask(bitField, 255);
    assert(find_alloc_freelist_index(bitField, 8161) == 255);  // Code has if non_zero_mask==0 return 255, so yes

    // Test 27: set every 8th bit, check skips correctly
    std::print("Test 27: set every 8th bit, check skips correctly\n");
    ResetBitField(bitField);
    for (int i = 0; i < 256; i += 8) set_alloc_freelist_mask(bitField, i);
    unsigned req5 = 32 * 5 + 1;  // requiredBin=5
    assert(find_alloc_freelist_index(bitField, req5) == 8);  // Next set is 8 >=5

    // Test 28: only bit 255 set, for various requiredBin
    std::print("Test 28: only bit 255 set, for various requiredBin\n");
    ResetBitField(bitField);
    assert(find_alloc_freelist_index(bitField, 1) == 255);
    assert(find_alloc_freelist_index(bitField, 4000) == 255);
    assert(find_alloc_freelist_index(bitField, 10000) == 255);
    ...
    ResetBitField(bitField);
    assert(find_alloc_freelist_index(bitField, 1ULL << 30) == 255);
