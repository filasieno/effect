...
static void ResetBitField(char* bitField) {
    ...
    priv::SetFreeListBit(bitField, 255);
}
...
DefineTask MainTask() noexcept {
    ...
    ResetBitField(bitField);
    priv::SetFreeListBit(bitField, 0);
    priv::SetFreeListBit(bitField, 1);
    priv::ClearFreeListBit(bitField, 0);
    assert(!priv::GetFreeListBit(bitField, 0));
    assert(priv::GetFreeListBit(bitField, 1));
    assert(priv::FindFreeListBucket(1, bitField) == 1);
    ...
    ResetBitField(bitField);
    for (int i = 0; i <= 10; ++i) priv::SetFreeListBit(bitField, i);
    priv::ClearFreeListBit(bitField, 2);
    priv::ClearFreeListBit(bitField, 4);
    priv::ClearFreeListBit(bitField, 6);
    priv::ClearFreeListBit(bitField, 8);
    unsigned req1 = 1;  // requiredBin=0
    assert(priv::FindFreeListBucket(req1, bitField) == 0);  // 0 is set
    unsigned req3 = 33;  // requiredBin=1
    assert(priv::FindFreeListBucket(req3, bitField) == 1);  // 1 set, 2 cleared

    // Test 21: requiredBin=127, bit127 set (end of second word)
    std::print("Test 21: requiredBin=127, bit127 set (end of second word)\n");
    ResetBitField(bitField);
    priv::SetFreeListBit(bitField, 127);
    unsigned req127 = 32 * 127 + 1;
    assert(priv::FindFreeListBucket(req127, bitField) == 127);

    // Test 22: requiredBin=128, bit128 set (start of third word)
    std::print("Test 22: requiredBin=128, bit128 set (start of third word)\n");
    ResetBitField(bitField);
    priv::SetFreeListBit(bitField, 128);
    unsigned req128 = 32 * 128 + 1;
    assert(priv::FindFreeListBucket(req128, bitField) == 128);

    // Test 23: no bits set from 200 to 254, falls to 255
    std::print("Test 23: no bits set from 200 to 254, falls to 255\n");
    ResetBitField(bitField);
    for (int i = 0; i < 200; ++i) priv::SetFreeListBit(bitField, i);
    unsigned req200 = 32 * 200 + 1;
    assert(priv::FindFreeListBucket(req200, bitField) == 255);  // since 200-254 not set

    // Test 24: verify Get after Set and Clear across bytes
    std::print("Test 24: verify Get after Set and Clear across bytes\n");
    ResetBitField(bitField);
    priv::SetFreeListBit(bitField, 7);   // last bit in byte 0
    priv::SetFreeListBit(bitField, 8);   // first in byte 1
    assert(priv::GetFreeListBit(bitField, 7));
    assert(priv::GetFreeListBit(bitField, 8));
    priv::ClearFreeListBit(bitField, 7);
    assert(!priv::GetFreeListBit(bitField, 7));
    assert(priv::GetFreeListBit(bitField, 8));
    ...
    ResetBitField(bitField);
    priv::ClearFreeListBit(bitField, 255);  // Temporarily clear wild for test (though design says always set)
    assert(priv::FindFreeListBucket(0, bitField) == 255);  // Should return 255 even if cleared, per code
    priv::SetFreeListBit(bitField, 255);  // Restore
    ...
    ResetBitField(bitField);
    priv::ClearFreeListBit(bitField, 255);
    assert(priv::FindFreeListBucket(8161, bitField) == 255);  // Code has if non_zero_mask==0 return 255, so yes

    // Test 27: set every 8th bit, check skips correctly
    std::print("Test 27: set every 8th bit, check skips correctly\n");
    ResetBitField(bitField);
    for (int i = 0; i < 256; i += 8) priv::SetFreeListBit(bitField, i);
    unsigned req5 = 32 * 5 + 1;  // requiredBin=5
    assert(priv::FindFreeListBucket(req5, bitField) == 8);  // Next set is 8 >=5

    // Test 28: only bit 255 set, for various requiredBin
    std::print("Test 28: only bit 255 set, for various requiredBin\n");
    ResetBitField(bitField);
    assert(priv::FindFreeListBucket(1, bitField) == 255);
    assert(priv::FindFreeListBucket(4000, bitField) == 255);
    assert(priv::FindFreeListBucket(10000, bitField) == 255);
    ...
    ResetBitField(bitField);
    assert(priv::FindFreeListBucket(1ULL << 30, bitField) == 255);
