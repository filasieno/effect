...
int parse_transport(lspd_config *out_config) 
{
    ...
    if (args.sock->count) {
        ...
        if (args.named->count == 0 && args.addr->count == 0) {
            out_config->transport.type = LSPD_TRANSPORT_TYPE_TCP_SOCKET;
        ...
        if (args.named->count) {
            ...
            out_config->transport.type = LSPD_TRANSPORT_TYPE_UNIX_SOCKET;
...
int lspd_parse_args(int argc, char **argv, lspd_config *out_config) 
{
    ...
    if (args.help->count) {
        ...
        out_config->transport.type = LSPD_TRANSPORT_TYPE_INVALID;
        return 0;
    }
