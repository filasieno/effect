...
TEST_F(AllocDefragTest, SmallBlockIntoSmallBlock) {
	AkVoid* p1 = alloc_table_try_malloc(&table, 32);
	AkVoid* p2 = alloc_table_try_malloc(&table, 32);
	...
	AkI32 defrag = alloc_table_defrag(&table, /*millis_budget*/0);
...
TEST_F(AllocDefragTest, SmallBlockIntoWildBlock) {
	...
	AkVoid* p = alloc_table_try_malloc(&table, 64);
	ASSERT_NE(p, nullptr);
	...
	AkI32 defrag = alloc_table_defrag(&table, 0);
	EXPECT_GE(defrag, 1);
...
TEST_F(AllocDefragTest, ManySmallBlocksToTreeBlock) {
	...
	for (int i = 0; i < kBlocks; ++i) {
		AkVoid* p = alloc_table_try_malloc(&table, 32);
		ASSERT_NE(p, nullptr);
		alloc_table_free(&table, p, 0);
	}
	AkU64 before_nodes = sum_freelist_nodes(&table);
...
TEST_F(AllocDefragTest, StatsConsistency) {
	...
	for (int i = 0; i < 16; ++i) {
		AkVoid* p = alloc_table_try_malloc(&table, 128);
		ASSERT_NE(p, nullptr);
		alloc_table_free(&table, p, 0);
	}
	AkI32 defrag = alloc_table_defrag(&table, 0);
