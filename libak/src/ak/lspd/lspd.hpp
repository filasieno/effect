...
enum class ldpd_transport_type {
...
inline int describe_transport(const LSPDTransport* t, char* buffer, size_t buffer_size) noexcept {
    switch (t->type) {
    case ldpd_transport_type::LSPD_TRANSPORT_TYPE_INVALID:
        return std::snprintf(buffer, buffer_size, "INVALID");
    case ldpd_transport_type::LSPD_TRANSPORT_TYPE_STDIO:
        return std::snprintf(buffer, buffer_size, "STDIO");
    case ldpd_transport_type::LSPD_TRANSPORT_TYPE_UNIX_SOCKET:
        return std::snprintf(buffer, buffer_size, "UNIX_SOCKET name: '%s'", t->file_name);
    case ldpd_transport_type::LSPD_TRANSPORT_TYPE_TCP_SOCKET:
