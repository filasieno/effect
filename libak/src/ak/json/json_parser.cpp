...
namespace ak {
    ...
    {    
        if (head == end) {
            ...
            if (json_size == 0) return raise_error(session, "empty input");
    ...
    static JSONParserState state_object_first_attr(JSONParseSession* session, U32 sub_state, Char* head, Char* end, U64 json_size, U64 string_size) noexcept {
        if (head == end) return suspend_parser(session, state_object_first_attr, sub_state, json_size, string_size);
        Char c = *head;
        switch (c) {
            case '}':
            {
    ...
    static JSONParserState state_object_rest_attrs(JSONParseSession* session, U32 sub_state, Char* head, Char* end, U64 json_size, U64 string_size) noexcept {
        if (head == end) return suspend_parser(session, state_object_rest_attrs, sub_state, json_size, string_size);
        Char c = *head;
        switch (c) {
            case '}':
            {
    ...
    static JSONParserState state_array_first_value(JSONParseSession* session, U32 sub_state, Char* head, Char* end, U64 json_size, U64 string_size) noexcept {
        while (true) {
            if (head == end) return suspend_parser(session, state_array_first_value, sub_state, json_size, string_size);
            Char c = *head;
            switch (c) {
                case ']':
                {
    ...
    static JSONParserState state_list_rest_values(JSONParseSession* session, U32 sub_state, Char* head, Char* end, U64 json_size, U64 string_size) noexcept {
        if (head == end) return suspend_parser(session, state_list_rest_values, sub_state, json_size, string_size);
        Char c = *head;
        switch (c) {
            case ',':
            {
    ...
    static JSONParserState state_attr_key_chars(JSONParseSession* session, U32 sub_state, Char* head, Char* end, U64 json_size, U64 string_size) noexcept 
    {
        ...
        while (true) {
            if (head == end) {
                // flush current chunk if any and suspend
                if (chunk_start != head) {
            ...
            if (c == '\\') {
                ...
                ++head; ++json_size;
                if (head == end) return suspend_parser(session, state_attr_key_chars, 1, json_size, string_size);
                Char e = *head;
                ++head; ++json_size;
                Char rc;
                switch (e) {
    ...
    static JSONParserState state_attr_begin_key(JSONParseSession* session, U32 sub_state, Char* head, Char* end, U64 json_size, U64 string_size) noexcept 
    {
        if (head == end) return suspend_parser(session, state_attr_begin_key, sub_state, json_size, string_size);
        Char c = *head;
        switch (c) {
            case '"':
            {
                ...
                AK_MUST_TAIL return state_attr_key_chars(session, sub_state, head, end, json_size, string_size);
            }
            case ' ': case '\t': case '\n': case '\r':
            {
                ++head; 
    ...
    static JSONParserState state_attr_semi(JSONParseSession* session, U32 sub_state, Char* head, Char* end, U64 json_size, U64 string_size) noexcept {
        (void)sub_state; (void)string_size;
        while (true) {
            if (head == end) return suspend_parser(session, state_attr_semi, 0, json_size, string_size);
            Char c = *head;
            switch (c) {
                case ':':
                {
    ...
    static JSONParserState state_value_dispatch(JSONParseSession* session, U32 sub_state, Char* head, Char* end, U64 json_size, U64 string_size) noexcept {
        (void)sub_state; (void)string_size;
        while (true) {
            if (head == end) return suspend_parser(session, state_value_dispatch, 0, json_size, string_size);
            Char c = *head;
            switch (c) {
                case ' ': case '\t': case '\n': case '\r':
                    ++head; ++json_size; continue;
    ...
    static JSONParserState state_null_head(JSONParseSession* session, U32 sub_state, Char* head, Char* end, U64 json_size, U64 string_size) noexcept {
        ...
        while (idx < 3) {
            if (head == end) return suspend_parser(session, state_null_head, idx, json_size, 0);
            Char c = *head;
            if (c != expected[idx]) return raise_error(session, "invalid token, expected 'null'");
            ++head; ++json_size; ++idx;
        }
    ...
    static JSONParserState state_true_head(JSONParseSession* session, U32 sub_state, Char* head, Char* end, U64 json_size, U64 string_size) noexcept {
        ...
        U32 idx = sub_state;
        while (idx < 3) {
            if (head == end) return suspend_parser(session, state_true_head, idx, json_size, 0);
            Char c = *head;
            if (c != expected[idx]) return raise_error(session, "invalid token, expected 'true'");
            ++head; ++json_size; ++idx;
        }
    ...
    static JSONParserState state_false_head(JSONParseSession* session, U32 sub_state, Char* head, Char* end, U64 json_size, U64 string_size) noexcept {
        ...
        U32 idx = sub_state;
        while (idx < 4) {
            if (head == end) return suspend_parser(session, state_false_head, idx, json_size, 0);
            Char c = *head;
            if (c != expected[idx]) return raise_error(session, "invalid token, expected 'false'");
            ++head; ++json_size; ++idx;
        }
    ...
    static JSONParserState state_number_head(JSONParseSession* session, U32 sub_state, Char* head, Char* end, U64 json_size, U64 string_size) noexcept {
        ...
        if (!is_float) {
            ...
            bool neg = false; U64 pos = 0; I64 val = 0;
            if (pos < session->suspend_buffer_size && session->suspend_buffer[pos] == '-') { neg = true; ++pos; }
            for (; pos < session->suspend_buffer_size; ++pos) {
                Char d = session->suspend_buffer[pos];
                if (d < '0' || d > '9') return raise_error(session, "invalid integer format");
                val = (val * 10) + (d - '0');
            }
            if (neg) val = -val;
    ...
    static JSONParserState state_string_head(JSONParseSession* session, U32 sub_state, Char* head, Char* end, U64 json_size, U64 string_size) noexcept {
        ...
        while (true) {
            ...
            if (c == '\\') {
                ...
                ++head; ++json_size;
                if (head == end) return suspend_parser(session, state_string_head, 1, json_size, string_size);
                ...
                notify_string_value_chars(session, &rc, 1);
                ++string_size;
                chunk_start = head;
                continue;
            } else if (c == '"') {
    ...
    // no resume_parser helper needed
