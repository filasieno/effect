...
AkVoid* alloc_table_try_malloc(AkAllocTable* at, AkSize size) noexcept {
    ...
    if (bin_idx >= 0) {
        ...
        AkAllocBlockHeader* next_block = alloc_block_next(block);
    ...
    // ======================================
    {
        ...
        AkAllocBlockHeader* next_block = alloc_block_next(old_wild);
        __builtin_prefetch(next_block, 1, 3);
...
AkVoid alloc_table_free(AkAllocTable* at, AkVoid* ptr, AkU32 side_coalescing) noexcept {
    ...
    // --------------------------
    AkAllocBlockHeader* next_block = alloc_block_next((AkAllocBlockHeader*)block);
...
AkI64 alloc_table_coalesce_left(AkAllocTable* at, AkAllocBlockHeader** out_block, AkU32 max_merges) noexcept {
    ...
    AkI64 merged = 0;
    while (max_merges--) {
        AkAllocBlockHeader* left = alloc_block_prev(block);
        ...
        AkAllocBlockHeader* right = alloc_block_next((AkAllocBlockHeader*)block);
...
AkI64 alloc_table_coalesce_right(AkAllocTable* at, AkAllocBlockHeader** out_block, AkU32 max_merges) noexcept {
    ...
    AkI64 merged = 0;
    while (max_merges--) {
        AkAllocBlockHeader* right = alloc_block_next(block);
        ...
        block->this_desc.size = new_size;
        AkAllocBlockHeader* right_right = alloc_block_next(block);
...
AkI32 alloc_table_defrag(AkAllocTable* at, AkU64 millis_budget) noexcept {
    ...
    for (AkAllocBlockHeader* h = begin; h != end; h = alloc_block_next(h)) {
