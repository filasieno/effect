...
void init_frame_pool(FramePool *framePool, U32 capacity, AllocTable *at) noexcept {
    U32 aligned_capacity = std::bit_ceil(static_cast<U32>(capacity));
    Size byte_size = static_cast<Size>(aligned_capacity) * sizeof(FrameId);
...
void init_frame_table(FrameTable *ft, U32 capacity, AllocTable *at) noexcept {
    ...
    U32 aligned = std::bit_ceil(static_cast<U32>(capacity));
    Size byte_size = static_cast<Size>(aligned) * sizeof(FrameEntry);
...
void dump_frame_table_debug(const FrameTable *ft) noexcept {
    ...
    for (U32 i = 0; i < ft->free_pool.count; ++i) {
        std::print("      frame_id: {}\n", ft->free_pool.entries[i].id);
    }
    std::print("    Default pool size: {}\n", ft->default_pool.count);
    for (U32 i = 0; i < ft->default_pool.count; ++i) {
        std::print("      frame_id: {}\n", ft->default_pool.entries[i].id);
    }
    std::print("    Keep pool size: {}\n", ft->keep_pool.count);
    for (U32 i = 0; i < ft->keep_pool.count; ++i) {
        std::print("      frame_id: {}\n", ft->keep_pool.entries[i].id);
    }
    std::print("    Recycle pool size: {}\n", ft->recycle_pool.count);
    for (U32 i = 0; i < ft->recycle_pool.count; ++i) {
        std::print("      frame_id: {}\n", ft->recycle_pool.entries[i].id);
    }
    std::print("  Entries\n");
    U32 cap = get_frame_table_capacity(ft);
    for (U32 i = 0; i < cap; ++i) {
        const FrameEntry &e = ft->entries[i];
        FrameId frame_id(i);
        if (e.pool == static_cast<U32>(BufferPool::INVALID)) {
            std::print("    {: >5} | free\n", frame_id.id);
            continue;
        }
        std::print("    {: >5} | {: >8} -> is_dirty: {} | evict:{} | pins: {} | pool_index: {} | p_bucket: {} | "
                   "vp_bucket: {}\n",
                   frame_id.id, ak::to_string(static_cast<BufferPool>(e.pool)), (Bool)e.is_dirty, (Bool)e.evict,
                   (U32)e.pin_count, (U32)e.pool_index.id, (U32)e.page_cache_bucket.id, (U32)e.vpage_cache_bucket.id);
    }
}
...
void free_frame(FrameTable *ft, FrameId frame_id) noexcept {
    ...
    AK_ASSERT(frame_entry->vpage_cache_bucket == VPageId(std::numeric_limits<U32>::max()));
    ...
    U32 src_pool_index = frame_entry->pool_index.id;
    ...
    frame_entry->pool = static_cast<U32>(BufferPool::INVALID);
    frame_entry->evict = 0;
    dest_pool->count++;
}

U32 get_frame_table_capacity(const FrameTable *ft) noexcept { return ft->free_pool.capacity; }

U32 get_frame_table_free_count(const FrameTable *ft) noexcept { return ft->free_pool.count; }
...
void move_frame_to_pool(FrameTable *ft, FrameId frame_id, BufferPool dest_pool_type) noexcept {
    ...
    FrameEntry &entry = ft->entries[frame_id.id];
    if (entry.pool == static_cast<U32>(dest_pool_type))
        return;
    ...
    U32 src_pool_index = entry.pool_index.id;
    AK_ASSERT(src_pool->entries[src_pool_index] == frame_id);
    ...
    entry.pool = static_cast<U32>(dest_pool_type);
    dest_pool->count++;
}

void check_invariants_free_pool(const FrameTable *ft) noexcept {
    for (U32 idx = 0; idx < ft->free_pool.count; ++idx) {
        FrameId frame_id = ft->free_pool.entries[idx];
        const FrameEntry &e = ft->entries[frame_id.id];
        AK_ASSERT(e.pool == static_cast<U32>(BufferPool::INVALID));
        AK_ASSERT(e.pool_index.id == idx);
    }
}

void check_invariants_keep_pool(const FrameTable *ft) noexcept {
    for (U32 idx = 0; idx < ft->keep_pool.count; ++idx) {
        FrameId frame_id = ft->keep_pool.entries[idx];
        const FrameEntry &e = ft->entries[frame_id.id];
        AK_ASSERT(e.pool == static_cast<U32>(BufferPool::KEEP));
        AK_ASSERT(e.pool_index.id == idx);
    }
}

void check_invariants_recycle_pool(const FrameTable *ft) noexcept {
    for (U32 idx = 0; idx < ft->recycle_pool.count; ++idx) {
        FrameId frame_id = ft->recycle_pool.entries[idx];
        const FrameEntry &e = ft->entries[frame_id.id];
        AK_ASSERT(e.pool == static_cast<U32>(BufferPool::RECYCLE));
        AK_ASSERT(e.pool_index.id == idx);
    }
}

void check_invariants_pool_capacity(const FrameTable *ft) noexcept {
    U32 capacity = get_frame_table_capacity(ft);
    U32 pools_sum = ft->default_pool.count + ft->free_pool.count + ft->keep_pool.count + ft->recycle_pool.count;
